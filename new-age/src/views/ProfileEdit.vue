<template>
  <div class="main">
    ...
    <form class="signup-form" @submit.prevent="editInfoHandler">
  ...
  methods: {
    async fetchUser() {
      ...
      if (user.firstName.length === 0) {
        ...
        const paramsId = this.$route.params.userId;
        const { data } = await getUser(token);
        this.$store.dispatch("setUser", data);
        if (paramsId !== data.id) {
          this.$router.push("/profile/" + data.id + "/edit");
        }
      }
